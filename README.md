...
**완료**
- 데이터 파이프라인 3종 + RAG(751건) + SQL 통계 + 연락처 매핑
- LangGraph 파이프라인 (parse→route→stats/retrieve→gate→respond/escalate)
- SSE 스트리밍 + Solar API 실연동, E2E 시나리오 3종(예방형/반응형/에스컬레이션) 검증 완료
- LLM 3단 방어 (timeout/재시도/강등)
- Langfuse 트레이싱 (LLM 호출 + LangGraph 노드별 실행 흐름)
- Docker + CI/CD (lint/test/DB마이그레이션/이미지빌드)
- GCE 실배포 완료 (405 이슈 해결)

**다음 단계**
- 평가셋 30건 구축 + precision/recall 측정 (DeepEval) — 최우선
- 2차 게이트 (LLM 적합성 판정) 추가 — 평가셋과 함께 진행
- LangGraph 체크포인터(AsyncPostgresSaver) 연동
- nginx 리버스 프록시로 포트 정리 (80 -> 8080 라우팅)
- 생활안전 API 승인분 등 추가 데이터 보강
...
- 필수 환경변수는 `DATABASE_URL`, `UPSTAGE_API_KEY` 둘뿐 (데이터 수집용 API 키들은 런타임에 불필요)
